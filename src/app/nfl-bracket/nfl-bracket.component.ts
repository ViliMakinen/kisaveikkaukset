...
export class NflBracketComponent implements OnInit {
  ...
  pairTeamsBySeed(teams: SeededTeam[]): SeededPair[] {
    ...
    while (tempWildCards.length > 0) {
      pairs.push({
        teams: [tempWildCards[0], tempWildCards[tempWildCards.length - 1]],
      });
      tempWildCards.shift();
      tempWildCards.pop();
    }
  ...
  selectAFCchampionships(team: SeededTeam, pair: SeededPair) {
    if (this.bracketPredictions.nflBracket.AFCchampionship.teams.includes(team)) {
      ...
      this.bracketPredictions.nflBracket.AFCchampionship.teams.push({
        name: '',
        seed: 0,
      });
    } else {
  ...
  isAFCDivisional(team: SeededTeam) {
    return (
      this.tempAFCdivisionals.map((tempTeam) => tempTeam.name).includes(team.name) ||
      this.bracketPredictions.nflBracket.AFCdivisionals.flatMap((pair) => pair.teams)
        .map((team) => team.name)
        .includes(team.name)
    );
  }
  ...
  isNFCDivisional(team: SeededTeam) {
    return (
      this.tempNFCdivisionals.map((tempTeam) => tempTeam.name).includes(team.name) ||
      this.bracketPredictions.nflBracket.NFCdivisionals.flatMap((pair) => pair.teams)
        .map((team) => team.name)
        .includes(team.name)
    );
  }
  ...
  selectNFCchampionships(team: SeededTeam, pair: SeededPair) {
    if (this.bracketPredictions.nflBracket.NFCchampionship.teams.includes(team)) {
      ...
      this.bracketPredictions.nflBracket.NFCchampionship.teams.push({
        name: '',
        seed: 0,
      });
    } else {
  ...
  getBackgroundColor(results: string[], playerPredictions: string[], team: SeededTeam) {
    if (playerPredictions.includes('') || results.includes('')) {
      return 'white';
    } else if (!results.includes(team.name) && playerPredictions.includes(team.name)) {
      return 'red';
    } else if (results.includes(team.name) && playerPredictions.includes(team.name)) {
      return 'green';
    }
  ...
  getDivisionalResults(team: SeededTeam, code: 'AFCdivisionals' | 'NFCdivisionals'): string {
    ...
    const playerDivisionalPredictions = (this.player.predictions.find((tournament) => tournament.tournament === 'NFL-playoffs')!.predictions as NFLBracket).nflBracket[code].flatMap((seededPair) =>
      seededPair.teams.map((team) => team.name),
    );
    return this.getBackgroundColor(divisionalWinners, playerDivisionalPredictions, team);
  }
  ...
  getChampionshipResults(team: SeededTeam, code: 'AFCchampionship' | 'NFCchampionship'): string {
    ...
    const playerChampionshipPredictions = (this.player.predictions.find((tournament) => tournament.tournament === 'NFL-playoffs')!.predictions as NFLBracket).nflBracket[code].teams.flatMap(
      (team) => team.name,
    );
    return this.getBackgroundColor(championshipWinners, playerChampionshipPredictions, team);
  }

  getSuperbowlResults(team: SeededTeam): string {
    const superbowlists = this.results.nflBracket.superbowlists.map((team) => team.name);
    const playerSuperbowlists = (this.player.predictions.find((tournament) => tournament.tournament === 'NFL-playoffs')!.predictions as NFLBracket).nflBracket.superbowlists.flatMap(
      (team) => team.name,
    );
    return this.getBackgroundColor(superbowlists, playerSuperbowlists, team);
  }
  ...
  getWinner(): string {
    ...
    if (superbowlWinner === '' || playerWinnerPrediction === '') {
      return 'white';
